...
public final class LevelStem {
   ...
   public static MappedRegistry<LevelStem> sortMap(MappedRegistry<LevelStem> pMappedRegistry) {
      ...
      for(ResourceKey<LevelStem> resourcekey : BUILTIN_ORDER) {
         ...
         if (levelstem != null) {
            mappedregistry.register(resourcekey, levelstem, pMappedRegistry.lifecycle(levelstem));
         }
      }
      ...
      for(Entry<ResourceKey<LevelStem>, LevelStem> entry : pMappedRegistry.entrySet()) {
         ...
         if (!BUILTIN_ORDER.contains(resourcekey1)) {
            mappedregistry.register(resourcekey1, entry.getValue(), pMappedRegistry.lifecycle(entry.getValue()));
         }
      }
   ...
   public static boolean stable(long pSeed, MappedRegistry<LevelStem> pMappedRegistry) {
      List<Entry<ResourceKey<LevelStem>, LevelStem>> list = Lists.newArrayList(pMappedRegistry.entrySet());
