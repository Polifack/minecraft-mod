...
public class LevelStorageSource {
   ...
   private static <T> Pair<WorldGenSettings, Lifecycle> readWorldGenSettings(Dynamic<T> pNbt, DataFixer pFixer, int pVersion) {
      ...
      return Pair.of(dataresult.resultOrPartial(Util.prefix("WorldGenSettings: ", LOGGER::error)).orElseGet(() -> {
         RegistryAccess registryaccess = RegistryAccess.RegistryHolder.readFromDisk(dynamic1);
   ...
   private static BiFunction<File, DataFixer, PrimaryLevelData> getReader(DynamicOps<Tag> pNbt, DataPackConfig pDatapackCodec, @Nullable LevelStorageAccess levelSave) {
      return (p_193010_, p_193011_) -> {
         try {
            CompoundTag compoundtag = NbtIo.readCompressed(p_193010_);
            CompoundTag compoundtag1 = compoundtag.getCompound("Data");
            CompoundTag compoundtag2 = compoundtag1.contains("Player", 10) ? compoundtag1.getCompound("Player") : null;
            compoundtag1.remove("Player");
            int i = compoundtag1.contains("DataVersion", 99) ? compoundtag1.getInt("DataVersion") : -1;
            Dynamic<Tag> dynamic = p_193011_.update(DataFixTypes.LEVEL.getType(), new Dynamic<>(pNbt, compoundtag1), i, SharedConstants.getCurrentVersion().getWorldVersion());
            Pair<WorldGenSettings, Lifecycle> pair = readWorldGenSettings(dynamic, p_193011_, i);
            LevelVersion levelversion = LevelVersion.parse(dynamic);
            LevelSettings levelsettings = LevelSettings.parse(dynamic, pDatapackCodec);
            PrimaryLevelData info = PrimaryLevelData.parse(dynamic, p_193011_, i, compoundtag2, levelsettings, levelversion, pair.getFirst(), pair.getSecond());
            if (levelSave != null)
               net.minecraftforge.internal.WorldPersistenceHooks.handleWorldDataLoad(levelSave, info, compoundtag);
            return info;
         } catch (Exception exception) {
            LOGGER.error("Exception reading {}", p_193010_, exception);
            return null;
         }
      };
   }
   ...
   BiFunction<File, DataFixer, LevelSummary> levelSummaryReader(File pSaveDir, boolean pLocked) {
      return (p_193015_, p_193016_) -> {
         try {
            CompoundTag compoundtag = NbtIo.readCompressed(p_193015_);
            CompoundTag compoundtag1 = compoundtag.getCompound("Data");
            compoundtag1.remove("Player");
            int i = compoundtag1.contains("DataVersion", 99) ? compoundtag1.getInt("DataVersion") : -1;
   ...
   public class LevelStorageAccess implements AutoCloseable {
      ...
      public Path getDimensionPath(ResourceKey<Level> pDimensionPath) {
         return DimensionType.getStorageFolder(pDimensionPath, this.levelPath);
      }
      ...
      public void saveDataTag(RegistryAccess pRegistries, WorldData pServerConfiguration, @Nullable CompoundTag pHostPlayerNBT) {
         ...
         net.minecraftforge.internal.WorldPersistenceHooks.handleWorldDataSave(this, pServerConfiguration, compoundtag1);
