...
public class ObjectiveArgument implements ArgumentType<String> {
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> pContext, SuggestionsBuilder pBuilder) {
      if (pContext.getSource() instanceof CommandSourceStack) {
         return SharedSuggestionProvider.suggest(((CommandSourceStack)pContext.getSource()).getServer().getScoreboard().getObjectiveNames(), pBuilder);
      } else if (pContext.getSource() instanceof SharedSuggestionProvider) {
         SharedSuggestionProvider sharedsuggestionprovider = (SharedSuggestionProvider)pContext.getSource();
         return sharedsuggestionprovider.customSuggestion((CommandContext<SharedSuggestionProvider>)pContext, pBuilder);
      } else {
